...
for i = 1:length(W)
   if W(i) >= state1(1) && W(i) < state1(2)
       S(i) = 1;
   elseif W(i) >= state2(1) && W(i) < state2(2)
       S(i) = 2;
   elseif W(i) >= state3(1) && W(i) < state3(2)
       S(i) = 3;
   elseif W(i) >= state4(1) && W(i) < state4(2)
       S(i) = 4;
   elseif W(i) >= state5(1) && W(i) <= state5(2)
       S(i) = 5;
   end
end
...
[idx1, val1] = find(S == 1);
S1_n = zeros(length(val1), 1);
for i = 1:length(val1)-1
    S1_n(i) = S(idx1(i) + 1);
end

% find the next state after state 2
[idx2, val2] = find(S == 2);
S2_n = zeros(length(val2), 1);
for i = 1:length(val2)
    S2_n(i) = S(idx2(i) + 1);
end
